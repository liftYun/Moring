...
Face Recognition, Drowsiness Detection, Approval System
"""
...
try:
    ...
    logging.warning("MQTT library not installed. pip install paho-mqtt")

# Logging setup - UTF-8 encoding support
logging.basicConfig(
    level=logging.INFO,
...
def put_korean_text(frame: np.ndarray, text: str, position: Tuple[int, int], 
    ...
    """Display Korean text on OpenCV frame (font cache applied)"""
    try:
        # Font cache key
        ...
        if font_key in _korean_font_cache:
            ...
            # Korean font loading (with cache)
            font = None
            font_paths = [
                # Windows fonts
            ...
            for font_path in font_paths:
                try:
                    ...
                    logging.info(f"Korean font loaded successfully (cached): {font_path}")
                    break
            ...
            if font is None:
                ...
                _korean_font_cache[font_key] = font
                logging.warning("Korean font not found, using default font")
        ...
        logging.warning(f"Korean text display failed, using English: {e}")
        # 실패 시 영어로 표시
...
class AutomationConfig:
    ...
    mqtt_broker_host: str = "192.168.10.3"  # 외부 브로커 사용
...
class ApprovalConfig:
    ...
    mqtt_broker: str = "192.168.10.3"  # 외부 MQTT 브로커
    mqtt_port: int = 1883
    mqtt_username: str = "moring"
    mqtt_password: str = "kimoring"
...
class SensorData:
    ...
    def __init__(self):
        ...
        self.sleep_avg: float = 0.0
        self.timestamp: str = ""
        self.driver_name: str = ""

class EnvironmentThresholds:
    """환경 임계값 클래스"""
    def __init__(self, co2_max: int = 1000, temp_max: float = 26.0, temp_min: float = 18.0, 
                 humidity_max: float = 70.0, sleep_avg_max: float = 0.7):
        self.co2_max = co2_max
        self.temp_max = temp_max
        self.temp_min = temp_min
        self.humidity_max = humidity_max
        self.sleep_avg_max = sleep_avg_max
...
class ConfigManager:
    ...
    def __init__(self, config_file: str = "config.json"):
        ...
        self.environment_thresholds = EnvironmentThresholds()
            ...
            if os.path.exists(self.config_file):
                ...
                if 'approval' in config_data:
                    ...
                    self.approval_config = ApprovalConfig(
                        ...
                        mqtt_broker=mqtt_config.get('broker', '43.203.216.22'),
                        mqtt_port=mqtt_config.get('port', 1883),
                        mqtt_username=mqtt_config.get('username', 'moring'),
                        mqtt_password=mqtt_config.get('password', 'kimoring'),
                        mqtt_client_id=mqtt_config.get('client_id_jetson', 'jetson_driver_monitor'),
                        approval_timeout=approval_data.get('approval_timeout', 60),
                        vin=vehicle_config.get('vin', 'KNMK5C2HMLP000437')
                    )
                ...
                if 'environment' in config_data:
                    ...
                    self.environment_thresholds = EnvironmentThresholds(
                        co2_max=env_data.get('co2_max', 1000),
                        temp_max=env_data.get('temp_max', 26.0),
                        temp_min=env_data.get('temp_min', 18.0),
                        humidity_max=env_data.get('humidity_max', 70.0),
                        sleep_avg_max=env_data.get('sleep_avg_max', 0.7)
                    )
                
                logging.info("Configuration file loaded successfully")
            else:
                self.save_default_config()
                logging.info("Default configuration file created successfully")
                
        except Exception as e:
            logging.error(f"Configuration file load failed: {e}")
            self.save_default_config()
    ...
    def save_config(self):
        ...
        try:
            config_data = {
                ...
                'approval': {
                    'enable_approval_system': self.approval_config.enable_approval_system,
                    'mqtt_broker': self.approval_config.mqtt_broker,
                    'mqtt_port': self.approval_config.mqtt_port,
                    'mqtt_username': self.approval_config.mqtt_username,
                    'mqtt_password': self.approval_config.mqtt_password,
                    'mqtt_client_id': self.approval_config.mqtt_client_id,
                    'approval_timeout': self.approval_config.approval_timeout,
                    'vin': self.approval_config.vin
                },
                'environment': {
                    'co2_max': self.environment_thresholds.co2_max,
                    'temp_max': self.environment_thresholds.temp_max,
                    'temp_min': self.environment_thresholds.temp_min,
                    'humidity_max': self.environment_thresholds.humidity_max,
                    'sleep_avg_max': self.environment_thresholds.sleep_avg_max
                }
            }
...
class MQTTClient:
    ...
    def _initialize_mqtt(self):
        ...
        if not MQTT_AVAILABLE:
            logging.warning("MQTT library not available, MQTT functionality disabled.")
            return
        ...
        try:
            ...
            if self.config.approval_config.mqtt_username and self.config.approval_config.mqtt_password:
                self.client.username_pw_set(self.config.approval_config.mqtt_username, 
            ...
            # 연결
            self.client.connect(self.config.approval_config.mqtt_broker, 
                              self.config.approval_config.mqtt_port, 60)
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            # 센서 데이터 토픽 구독 (와일드카드 패턴)
            sensor_topic = f"car/{self.config.approval_config.vin}/sensor/#"
            client.subscribe(sensor_topic)
            logging.info(f"Subscribed to sensor data topic: {sensor_topic}")
            
            # 승인 응답 토픽 구독 (와일드카드 패턴)
            approval_topic = f"car/{self.config.approval_config.vin}/auth/#"
            client.subscribe(approval_topic)
            logging.info(f"Subscribed to approval response topic: {approval_topic}")
        else:
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            logging.info(f"MQTT message received: {topic}")
            
            if "sensor/data" in topic:
                self._handle_sensor_data(data)
            elif "auth/approval/response" in topic:
    ...
    def _on_disconnect(self, client, userdata, rc):
        """MQTT 연결 해제 콜백"""
        self.connected = False
        logging.warning(f"MQTT 연결 해제: {rc}")
    
    def _handle_sensor_data(self, data: Dict[str, any]):
        """센서 데이터 처리"""
        try:
            # 센서 데이터 업데이트
            self.config.sensor_data.driver_name = data.get("driver_name", "")
            self.config.sensor_data.co2 = data.get("co2", 0)
            self.config.sensor_data.temperature = data.get("temperature", 0.0)
            self.config.sensor_data.humidity = data.get("humidity", 0.0)
            self.config.sensor_data.sleep_count = data.get("sleep_count", 0)
            self.config.sensor_data.sleep_avg = data.get("sleep_avg", 0.0)
            self.config.sensor_data.timestamp = data.get("timestamp", "")
            
            logging.info(f"Sensor data updated: {self.config.sensor_data.driver_name} - CO2: {self.config.sensor_data.co2}, Temp: {self.config.sensor_data.temperature}")
            
        except Exception as e:
            logging.error(f"센서 데이터 처리 실패: {e}")
    
    def _handle_approval_response(self, data: Dict[str, any]):
        """승인 응답 처리"""
        try:
            session_id = data.get("sessionId", "")
            approval_result = data.get("result", "DENY")  # RPi에서 보낸 'result' 키 사용
            
            logging.info(f"Approval response received: SessionID {session_id} - {approval_result}")
            
            # ApprovalManager의 pending_approvals 업데이트
            if hasattr(self, 'approval_manager') and self.approval_manager:
                with self.approval_manager.approval_lock:
                    if session_id in self.approval_manager.pending_approvals:
                        approval_info = self.approval_manager.pending_approvals[session_id]
                        approval_info['decision'] = approval_result
                        approval_info['received_time'] = time.time()
                        logging.info(f"승인 처리 완료: {approval_result}")
                    else:
                        logging.warning(f"알 수 없는 sessionId: {session_id}")
            
            # 승인 결과에 따른 처리
            if approval_result == "APPROVE":
                logging.info(f"Approved: SessionID {session_id}")
                # 승인된 경우 처리 로직
                self.approval_requested = False  # 승인 요청 플래그 리셋
            elif approval_result == "DENY":
                logging.info(f"Denied: SessionID {session_id}")
                # 거부된 경우 처리 로직
                self.approval_requested = False  # 승인 요청 플래그 리셋
            else:
                logging.warning(f"Unknown approval result: {approval_result}")
            
        except Exception as e:
            logging.error(f"승인 응답 처리 실패: {e}")
    
    def send_sleep_alert(self, driver_name: str, sleep_count: int, sleep_avg: float):
        """졸음 알림 전송"""
        if not self.connected or not self.client:
            logging.warning("MQTT not connected, cannot send sleep alert.")
            return
            
        try:
            topic = f"car/{self.config.approval_config.vin}/alert/sleep"
            payload = {
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logging.info(f"졸음 알림 전송 완료: {driver_name}")
            else:
                logging.error(f"졸음 알림 전송 실패: {result.rc}")
                
        except Exception as e:
            logging.error(f"졸음 알림 전송 실패: {e}")
    
    def send_unauthorized_alert(self, driver_name: str):
        """비인가 사용자 알림 전송"""
        if not self.connected or not self.client:
            logging.warning("MQTT가 연결되지 않아 비인가 알림을 전송할 수 없습니다.")
            return
            
        try:
            topic = f"car/{self.config.approval_config.vin}/alert/unauthorized"
            payload = {
                "driver_name": driver_name,
                "unauthorized_detected": True,
                "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                "s3_image_url" : "https://example.com/unauthorized_user.jpg"

            }
            
            result = self.client.publish(topic, json.dumps(payload))
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logging.info(f"비인가 알림 전송 완료: {driver_name}")
            else:
                logging.error(f"비인가 알림 전송 실패: {result.rc}")
                
        except Exception as e:
            logging.error(f"비인가 알림 전송 실패: {e}")
    
    def send_distraction_alert(self, driver_name: str):
        """시선 이탈 알림 전송"""
        if not self.connected or not self.client:
            logging.warning("MQTT가 연결되지 않아 시선 이탈 알림을 전송할 수 없습니다.")
            return
            
        try:
            topic = f"car/{self.config.approval_config.vin}/alert/distraction"
            payload = {
                "driver_name": driver_name,
            ...
            result = self.client.publish(topic, json.dumps(payload))
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logging.info(f"시선 이탈 알림 전송 완료: {driver_name}")
            else:
                logging.error(f"시선 이탈 알림 전송 실패: {result.rc}")
                
        except Exception as e:
            logging.error(f"시선 이탈 알림 전송 실패: {e}")
...
class EnvironmentMonitor:
    ...
    def check_environment_status(self, driver_name: str) -> Dict[str, any]:
        """환경 상태 체크"""
        try:
            sensor_data = self.config.sensor_data
            thresholds = self.config.environment_thresholds
            
            # 현재 운전자와 센서 데이터의 운전자가 일치하는지 확인
            if sensor_data.driver_name != driver_name:
                return {
                    'status': 'unknown',
            ...
            # CO2 체크
            if sensor_data.co2 > thresholds.co2_max:
                alerts.append({
                    'type': 'co2_high',
                    'message': f'이산화탄소 농도가 높습니다 ({sensor_data.co2}ppm). 환기를 시켜주세요.',
                    'value': sensor_data.co2,
                    'threshold': thresholds.co2_max
                })
            ...
            if sensor_data.temperature > thresholds.temp_max:
                alerts.append({
                    'type': 'temp_high',
                    'message': f'실내 온도가 높습니다 ({sensor_data.temperature}°C). 에어컨을 켜주세요.',
                ...
                alerts.append({
                    'type': 'temp_low',
                    'message': f'실내 온도가 낮습니다 ({sensor_data.temperature}°C). 난방을 켜주세요.',
                    'value': sensor_data.temperature,
            ...
            if sensor_data.humidity > thresholds.humidity_max:
                alerts.append({
                    'type': 'humidity_high',
                    'message': f'습도가 높습니다 ({sensor_data.humidity}%). 제습기를 켜주세요.',
                    'value': sensor_data.humidity,
                    'threshold': thresholds.humidity_max
                })
            
            # 졸음 평균값 체크
            if sensor_data.sleep_avg > thresholds.sleep_avg_max:
                alerts.append({
                    'type': 'sleep_high',
                    'message': f'졸음이 자주 감지됩니다 (평균: {sensor_data.sleep_avg:.2f}). 휴식을 취해주세요.',
                    'value': sensor_data.sleep_avg,
                    'threshold': thresholds.sleep_avg_max
                })
            ...
            if len(alerts) == 0:
                ...
                status = 'warning'
                message = '환경 개선이 필요합니다.'
            else:
                status = 'danger'
                message = '환경이 위험합니다. 즉시 조치가 필요합니다.'
            ...
            return {
                ...
                'sensor_data': {
                    ...
                    'sleep_avg': sensor_data.sleep_avg
                }
            }
            
        except Exception as e:
            logging.error(f"환경 상태 체크 실패: {e}")
            return {
                'status': 'error',
                'message': '환경 상태를 확인할 수 없습니다.',
                'alerts': []
            }
    ...
    def load_model(self):
        ...
        try:
            ...
            if hasattr(self.model, 'names'):
                logging.info(f"YOLO 모델 클래스: {list(self.model.names.values())}")
            ...
            logging.info(f"YOLO 모델 로드 완료: {self.model_path} ({file_size / (1024*1024):.1f}MB)")
                
        except FileNotFoundError as e:
            logging.error(f"모델 파일 오류: {e}")
            logging.error("models/best.pt 파일을 확인하세요")
            self.model = None
        except Exception as e:
...
class DetectionManager:
    ...
    def check_drowsiness(self, frame: np.ndarray, ear: float, pitch: float, current_time: float, baseline: dict = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 베이스라인이 없으면 감지하지 않음
            if not baseline or baseline.get("BASELINE_EYE_AR") is None:
                return False, "No baseline available"
            
            # 베이스라인 기반 임계값 계산
            eye_threshold = baseline["BASELINE_EYE_AR"] * 0.6
            pitch_threshold = baseline["BASELINE_PITCH"] * 2.0
    ...
    def check_distraction(self, frame: np.ndarray, yaw: float, roll: float, current_time: float, baseline: dict = None) -> Tuple[bool, str]:
        """시선 이탈 감지 - 베이스라인 기반만 사용"""
        try:
            gaze_deviation_detected = False
            terminal_message = ""
            
            # 베이스라인이 없으면 감지하지 않음
            if not baseline or baseline.get("BASELINE_YAW") is None:
                return False, "No baseline available"
            
            # 베이스라인 기반 임계값 계산
            yaw_threshold = baseline["BASELINE_YAW"] * 3.0
            roll_threshold = baseline["BASELINE_ROLL"] * 2.0
...
class ApprovalManager:
    ...
    def __init__(self, config: ConfigManager):
        ...
        # MQTT 클라이언트는 DriverMonitor에서 관리하므로 여기서는 초기화하지 않음
        # if MQTT_AVAILABLE and self.config.approval_config.enable_approval_system:
        #     self.initialize_mqtt()
    

    
    def request_approval(self, frame: np.ndarray, mqtt_client=None) -> str:
        """승인 요청"""
        try:
            # 세션 ID 생성
            ...
            request_data = {
                ...
                'snapshotUrl': 'data:image/jpeg;base64,test_image_data',  # 테스트용 이미지 URL
                'timestamp': int(time.time()),
                'vin': self.config.approval_config.vin
            }
            
            # MQTT로 요청 발송 (paho-mqtt 클라이언트 사용)
            request_topic = f"car/{self.config.approval_config.vin}/auth/approval/request"
            message = json.dumps(request_data)
            
            try:
                # 외부에서 전달받은 MQTT 클라이언트 사용
                if mqtt_client:
                    result = mqtt_client.publish(request_topic, message, qos=1)
                    if result.rc == mqtt.MQTT_ERR_SUCCESS:
                        ...
                        logging.error(f"MQTT 발송 실패: {result.rc}")
                        return "DENY"
                else:
                    logging.error("MQTT 클라이언트가 전달되지 않았습니다")
                    return "DENY"
                    
            except Exception as e:
                logging.error(f"MQTT 발송 실패: {e}")
                return "DENY"
    ...
    def cleanup(self):
        """정리"""
        # MQTT 클라이언트는 DriverMonitor에서 관리하므로 여기서는 정리하지 않음
        pass
...
class DisplayManager:
    ...
    def display_info_on_frame(self, frame: np.ndarray, screen_width: int, 
        ...
        try:
            ...
            # 환경 상태 정보 (센서 데이터가 있을 때만 표시)
            if hasattr(self.config, 'sensor_data') and self.config.sensor_data.driver_name == current_driver:
                sensor_data = self.config.sensor_data
                env_y = 215
                
                # 환경 상태 제목
                cv2.putText(frame, "Environment Status:", (screen_width - 310, env_y), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                
                # CO2 상태
                co2_color = (0, 255, 0) if sensor_data.co2 <= 1000 else (0, 255, 255) if sensor_data.co2 <= 1500 else (0, 0, 255)
                cv2.putText(frame, f"CO2: {sensor_data.co2}ppm", (screen_width - 310, env_y + 20), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, co2_color, 1)
                
                # 온도 상태
                temp_color = (0, 255, 0) if 18 <= sensor_data.temperature <= 26 else (0, 255, 255) if 15 <= sensor_data.temperature <= 30 else (0, 0, 255)
                cv2.putText(frame, f"Temp: {sensor_data.temperature:.1f}°C", (screen_width - 310, env_y + 35), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, temp_color, 1)
                
                # 습도 상태
                humidity_color = (0, 255, 0) if sensor_data.humidity <= 70 else (0, 255, 255) if sensor_data.humidity <= 80 else (0, 0, 255)
                cv2.putText(frame, f"Humidity: {sensor_data.humidity:.1f}%", (screen_width - 310, env_y + 50), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, humidity_color, 1)
                
                # 졸음 평균값
                sleep_color = (0, 255, 0) if sensor_data.sleep_avg <= 0.7 else (0, 255, 255) if sensor_data.sleep_avg <= 0.8 else (0, 0, 255)
                cv2.putText(frame, f"Sleep Avg: {sensor_data.sleep_avg:.2f}", (screen_width - 310, env_y + 65), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, sleep_color, 1)
...
class DriverMonitor:
    ...
    def __init__(self, config_file: str = "config.json"):
        ...
        self.baseline_duration = self.config.auto_setup_config.baseline_duration
        self.baseline_data = []
        
        # 자동 얼굴 등록 관련
        self.auto_face_registration_active = False
        self.registration_start_time = None
        self.registration_duration = self.config.auto_setup_config.registration_duration
        self.registration_data = []
        self.registration_user_name = self.config.auto_setup_config.default_user_name
    ...
    def auto_recognize_driver(self, frame: np.ndarray) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 미등록 사용자 또는 불확실한 매치인 경우 승인 요청 (한 번만)
            if identified_name in ["Unknown Driver", "Uncertain"]:
                # 이미 승인 요청 중인지 확인
                if not hasattr(self, 'approval_requested') or not self.approval_requested:
                    logging.warning(f"{identified_name} 감지 - 승인 요청 시작")
                    self.approval_requested = True
                    
                    # 승인 요청
                    decision = self.approval_manager.request_approval(frame, self.mqtt_client)
                    ...
                    if decision == "APPROVE":
                        ...
                        if self.face_processor.register_driver_face(frame, new_user_name):
                            ...
                            self.approval_requested = False  # 리셋
            ...
            if self.current_driver != identified_name:
                ...
                if identified_name in self.driver_baselines:
                    ...
                    print(f"[INFO] 베이스라인 기반 감지 시작: {identified_name}")
                else:
    ...
    def apply_personal_thresholds(self, driver_name: str):
        """개인별 임계치 적용 (베이스라인 기반만 사용)"""
        try:
            # 베이스라인 기반 임계값만 사용하므로 개인별 임계치 적용은 제거
            logging.info(f"베이스라인 기반 감지 사용: {driver_name}")
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, any]:
        ...
        try:
            ...
            if face_result['success']:
                ...
                # 졸음 감지 시 MQTT 알림 전송
                if drowsiness_detected and self.current_driver:
                    current_time = time.time()
                    # 알림 쿨다운 체크 (5초마다 한 번씩만)
                    if current_time - getattr(self, 'last_sleep_alert_time', 0) > 5:
                        sleep_count = self.detection_manager.counters.get('eyes_closed', 0)
                        sleep_avg = face_result['ear']  # 현재 EAR 값을 평균으로 사용
                        self.mqtt_client.send_sleep_alert(self.current_driver, sleep_count, sleep_avg)
                        self.last_sleep_alert_time = current_time
                        logging.info(f"졸음 알림 전송: {self.current_driver}")
                ...
                # 시선 이탈 연속 감지 기반 알림 (3초 연속 감지 시에만)
                if distraction_detected and self.current_driver:
                    if not hasattr(self, 'distraction_counter'):
                        self.distraction_counter = 0
                    if not hasattr(self, 'last_distraction_time'):
                        self.last_distraction_time = time.time()
                    
                    current_time = time.time()
                    if current_time - self.last_distraction_time < 1.0:  # 1초 이내면 카운터 증가
                        self.distraction_counter += 1
                    else:
                        self.distraction_counter = 1  # 리셋
                    
                    self.last_distraction_time = current_time
                    
                    # 3초 연속 감지 시에만 MQTT 알림 전송
                    if self.distraction_counter >= 3:
                        if current_time - getattr(self, 'last_distraction_alert_time', 0) > 10:  # 10초 쿨다운
                            self.mqtt_client.send_distraction_alert(self.current_driver)
                            self.last_distraction_alert_time = current_time
                            self.distraction_counter = 0  # 리셋
                            logging.info(f"시선 이탈 알림 전송: {self.current_driver}")
                
                # 시선 이탈이 없으면 카운터 리셋 (distraction_detected가 False일 때)
                if not distraction_detected and hasattr(self, 'distraction_counter'):
                    self.distraction_counter = 0
        ...
        try:
            ...
            if hasattr(self, 'mqtt_client'):
                try:
                    ...
                    logging.error(f"MQTT 클라이언트 정리 실패: {e}")
