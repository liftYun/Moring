                ...
                for name, emb in data.items():
                    try:
                        if isinstance(emb, list):
                            ...
                            loaded_faces[name] = np.array(emb)
                    except Exception as e:
...
class YOLODetector:
    ...
    def load_model(self):
        ...
        try:
            ...
            # GPU 사용 설정
            if self.config.performance_config.enable_gpu:
                self.model.to('cuda')
...
class DriverMonitor:
    ...
    def auto_recognize_driver(self, frame: np.ndarray) -> Tuple[bool, str]:
        ...
        try:
            ...
            if identified_name in ["No Face", "Multiple Faces", "Encoding Failed", "No Registered", "Unknown Driver", "Identification Error"]:
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, any]:
        ...
        try:
            ...
            if self.frame_counter % self.config.performance_config.face_recognition_interval == 0:
                if not self.baseline_measurement_active and not self.new_user_registration_active:
...
def run_driver_monitoring_system():
    ...
    try:
        ...
        if len(sys.argv) > 1 and '--test_mode' in sys.argv:
        ...
        # 얼굴 등록이 없으면 자동 등록 시작
        if not registered_faces and monitor.config.auto_setup_config.auto_face_registration:
            print("[INFO] No registered faces found. Starting auto face registration...")
            monitor.start_auto_face_registration()
        
        # 베이스라인이 없으면 자동 베이스라인 측정 시작
        if not baseline_exists and monitor.config.auto_setup_config.auto_baseline_measurement:
            print("[INFO] No baseline found. Starting auto baseline measurement...")
            user_name = monitor.config.auto_setup_config.default_user_name
            monitor.start_baseline_measurement(user_name, monitor.config.auto_setup_config.baseline_duration)
        ...
        while True:
            ...
            cv2.putText(frame, f"ID Status: {monitor.identification_status}", 
                       (50, screen_height - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            ...
            cv2.putText(frame, f"Performance: {'ON' if monitor.config.performance_config.performance_mode else 'OFF'}", 
                       (50, screen_height - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, perf_color, 1)
            cv2.putText(frame, f"FPS: {perf_info['fps']:.1f} | Skip:{monitor.config.performance_config.skip_frames}", 
                       (50, screen_height - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
