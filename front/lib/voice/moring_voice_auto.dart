// lib/voice/moring_voice_auto.dart
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
// 앱의 Dio 주입(없으면 대체용으로 _plainDio 사용)
// 아래 import는 실제 프로젝트 구조에 맞게 수정해야 할 수 있습니다.
// 만약 'moring/providers/api_client.dart' 파일이 없다면 이 라인을 주석 처리하고
// _callLlm 함수가 _plainDio를 사용하도록 하십시오.
...
/// ✅ OpenAI TTS (GMS 프록시)
const _gmsKey = 'S13P11E101-1b3cd263-2a3e-4c99-9fa5-8dd56eb86f62'; // GMS 토큰
...
class _VoiceAssistantPanelState extends ConsumerState<VoiceAssistantPanel>
  ...
  Future<void> _checkForWakeWord(String heardText) async {
    ...
    if (userText != null) {
      ...
      // TTS가 끝난 후 다음 턴에서 VAD가 idle을 bump하므로 여기서 arm할 필요 없음
    }
  }
  ...
  String? _extractAfterWakeWord(String text) {
    ...
    for (final w in _wakeWords) {
      if (noSpace.startsWith(w)) {
        int originalIndex = text.toLowerCase().indexOf(w);
        if(originalIndex != -1) {
  ...
  String? _extractLlmResult(Response res) {
    ...
    if (res.statusCode == 200) {
      final data = res.data;
      if (data is Map && data.containsKey('result') && data['result'] is String) return (data['result'] as String).trim();
    ...
    try {
      ...
      final res = await _plainDio.post(
        ...
        options: Options(
          ...
          responseType: ResponseType.bytes, // mp3 바이너리 응답
          validateStatus: (s) => s != null && s < 500,
        ),
      );
